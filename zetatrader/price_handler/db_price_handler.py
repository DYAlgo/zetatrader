...
class DbPriceHandler(AbstractPriceHandler):
    """[summary]
    """
    def __init__(self, events, symbol_dict, start_dt, end_dt
        , db_user=os.environ.get('SEC_DB_USER')
        , db_password=os.environ.get('SEC_DB_PW'), frequency='daily'
    ):
    ...
    def construct_symbol_data(self):
        ...
        for symbol in self.symbol_list:
            ...
            AND '%s' ) ORDER BY price_date ASC"""%(
                self.frequency , self.symbol_dict.get(symbol,0)
