...
class TestDbPriceHandler(unittest.TestCase):
    def test_construct_symbol_data(self):
        price_handler = DbPriceHandler(
            'MARKET_EVENT', {'AAPL': 3656, 'EWM': 7231} 
    ...
    def test_latest_bar(self):
        ...
        self.assertEqual(
            price_handler.get_latest_bar('EWM')['close_price'], 29.32
        )
        self.assertEqual(
            price_handler.get_latest_bar('AAPL')['close_price'], 209.15
        )
    ...
    def test_latest_bars(self):
        ...
        self.assertEqual(
            len(price_handler.get_latest_bars('AAPL', 5))
            , 5
        )
        
        self.assertEqual(
            len(price_handler.get_latest_bars('AAPL', 10))
            , 5
        )
    ...
    def test_get_latest_bar_datetime(self):
        ...
        self.assertEqual(
            price_handler.get_latest_bar_datetime('EWM')
    ...
    def test_get_latest_bar_value(self):
        ...
        self.assertEqual(
            price_handler.get_latest_bar_value('AAPL', 'close_price')
            , 190.92
        )
    ...
    def test_get_latest_bar_values(self):
        ...
        price_handler.bar_index = 10
        self.assertEqual(
            len(price_handler.get_latest_bars_values('AAPL', 'close_price', 11))
            , 11
        )
        self.assertEqual(
            len(price_handler.get_latest_bars_values('AAPL', 'close_price', 15))
            , 11
        )
